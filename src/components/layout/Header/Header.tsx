...
const Header = () => {
  ...
  const [shipTo, setShipTo] = useState({ country: 'US', flag: 'https://cdn.britannica.com/79/4479-050-6EF87027/flag-Stars-and-Stripes-May-1-1795.jpg' });
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center flex-wrap">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            {['profile', 'message', 'product', 'cart'].map((item) => (
              <a key={item} href={`${item}.html`} className="text-white flex items-center space-x-2">
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden bg-blue-600 text-white p-4">
            <div className="flex flex-col space-y-4">
              {['profile', 'message', 'product', 'cart'].map((item) => (
                <a key={item} href={`${item}.html`} className="flex items-center space-x-2">
                  <i className={`fas fa-${item === 'cart' ? 'shopping-cart' : item}`}></i>
                  <span>{item.charAt(0).toUpperCase() + item.slice(1)}</span>
                </a>
              ))}
            ...
            {isCategoriesMenuOpen && (
              <div className="absolute left-0 mt-2 w-48 bg-white shadow-lg rounded-lg">
                <ul className="flex flex-col">
                  {['Categories', 'Furniture', 'Watches', 'Smartphones', 'Ladies Clothes', 'Gents Clothes', 'Crockery', 'Jewellery', 'Electronics', 'Kids', 'Laptops'].map((category) => (
                    <li key={category}>
                      <a href="product.html" className="block px-4 py-2 text-gray-800">
                        {category}
                      </a>
                    </li>
                  ))}
